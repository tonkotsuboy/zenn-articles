...
`commandfor`属性で操作対象のポップオーバーのIDを指定し、`command`属性で動作（開く / 閉じる / 切替）を指定します。`popover`属性で要素をポップオーバーとして宣言します。

| 属性 | 役割 |
|------|------|
| `popover` | 要素をポップオーバーとして宣言 |
| `commandfor` | クリック時に操作する要素のIDを指定 |
| `command` | 動作を指定（`toggle-popover` / `show-popover` / `hide-popover`） |

なお、`commandfor`属性はHTML仕様上、`<button>`要素でのみ使用できます。`<a>`要素では動作しません。

:::message
`commandfor` / `command`は、従来の`popovertarget` / `popovertargetaction`の**汎用版・後継API**です（Invoker Commands API）。`popover`だけでなく`<dialog>`等も同じ2属性で操作でき、Baseline 2025で利用可能になりました。
:::
...
```html
<button class="button" commandfor="my-popover" command="toggle-popover">開く</button>
...
<nav class="menu">
  <ul class="menu-list">
    <li class="menu-item">
      <button class="menu-link" id="task-menu-anchor" commandfor="task-submenu" command="toggle-popover">
        タスク管理
...
前述のとおり、`commandfor`属性は`<button>`要素で使用します（`<a>`要素では動作しません）。
...
```html
<header class="header">
  <button commandfor="user-menu" command="toggle-popover" class="user-icon">
