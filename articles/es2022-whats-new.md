...
今回のようにあるオブジェクトが`MyClass`のデータかどうかをチェックすることは、プライベートフィールドと`in`演算子を使ってブランドチェックをしていると言えます。`in`を使ってシンプルにブランドチェックができるので、この提案の名前は「Ergonomic brand checks」（使いやすいブランドチェック）なのです。
